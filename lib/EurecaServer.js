            ...
            var createServer = function (hook, options = {}) {
                try {
                    ...
                    if (options.cookieParser)
                        primus.before('cookies', options.cookieParser);
                    if (options.session)
                        primus.before('session', options.session);
                    return server;
                }
                catch (ex) {
    ...
    (function (Transports) {
        ...
        (function (WebRTC) {
            ...
            class Peer extends Eureca.EObject {
                ...
                makeDataChannel() {
                    var __this = this;
                    var channel = this.pc.createDataChannel(this.id, { reliable: __this.channelSettings.reliable, maxRetransmits: __this.channelSettings.maxRetransmits, ordered: __this.channelSettings.ordered });
                ...
                doCreateAnswer() {
                    var __this = this;
                    this.pc.createAnswer(function (desc) { __this.doSetLocalDesc(desc); }, function (error) { __this.doHandleError(error); });
                }
                doSetLocalDesc(desc) {
                    var __this = this;
                    this.pc.setLocalDescription(desc, function () { }, function (error) { __this.doHandleError(error); });
                }
                doHandleError(error) {
                ...
                doHandleDataChannels() {
                    ...
                    this.pc.setRemoteDescription(this.offer, function () { __this.doCreateAnswer(); }, function (error) { __this.doHandleError(error); });
                }
            }
            WebRTC.Peer = Peer;
...
var Eureca;
(function (Eureca) {
    class Server extends Eureca.EObject {
        constructor(settings = {}) {
            super();
            this.settings = settings;
            this.scriptCache = '';
        ...
        _handleServer(ioServer) {
            ...
            ioServer.onconnect(function (socket) {
                ...
                var sendContract = function () {
                    __this.contract = Eureca.Contract.ensureContract(__this.exports, __this.contract);
                    var sendObj = {};
